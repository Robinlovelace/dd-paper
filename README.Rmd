...
names(l)
nrow(l_cities) / nrow(l) # 1.6%, 7000 OD pairs
summary(l_cities$all)
# l_cities = l_cities %>% filter(all >= 20) %>% 
l_cities = l %>% filter(all >= 50) %>% 
  select(city = id, all:taxi_other, distance_euclidean = e_dist_km, distance = rf_dist_km, gradient = rf_avslope_perc, dutch_slc) %>% 
  mutate(pcycle = bicycle / all, pwalk = foot / all)
unique(l_cities$city)
region_names_title = c("Cambridgeshire", "Avon", "West Yorkshire")
l_cities$city = as.factor(l_cities$city)
levels(l_cities$city) = region_names_title
unique(l_cities$city)
